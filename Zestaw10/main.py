...
import os
def onclick(player_input, player_img, player_label, bot_label, result_label):
    bot_label.randomize(player_input, result_label)
...
pc_answer = BotAnswer(left_panel, width=100, height=100, rock_img=rock_img, paper_img=paper_img, scissors_img=scissor_img)
...
k_button = tk.Button(buttons, text="K",width=20, height=2, background='gray', command=lambda: onclick(0,rock_img, ur_answer, pc_answer, result))
k_button.grid(column=0, row = 0)
p_button = tk.Button(buttons, text="P",width=20, height=2, command=lambda: onclick(1,paper_img,ur_answer,pc_answer, result))
p_button.grid(column=0, row = 1)
n_button = tk.Button(buttons, text="N",width=20, height=2, background='cyan', command=lambda: onclick(2,scissor_img,ur_answer,pc_answer, result))
